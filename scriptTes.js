...
//BUg : bila operator di klik duluan, lalu dihapus, hasil operasi matematika adalah Nan
//BUg : bila serelah memasukkan input1, operator di klik lebih dari sekali, lalu dihapus, hasil operasi matematika adalah Nan
//Bug : bila angka dihapus sampai 0, hasil operasi matematika adalah Nan
...
keyCalc.addEventListener('click', function (e) {
    ...
    if(!document.querySelector('.clear').classList.contains('delete') && operatorCounter < 2){
        
        inputKey += e.target.textContent;   //menampilkan input ke display calculator
    ...
    if(e.target.tagName=='BUTTON'){     //cek apakah button pernah di klik
        // if(e.target.textContent !=='AC'){
            ...
            if(e.target.textContent == 'CE'){   //cek apakah button CE yang dklik
        ...
        if(e.target.className !== 'key--equal' && e.target.className !== 'clear'){  //menangkap semua input yang di klik kecuali button = dan AC dan tampilkan ke display calculator 
            ...
            if(clickCounter > 0){
                ...
                for (let i = input1.length+1; i < preInput2.length; i++) { //pake cara ini, masih ada bug
                    input2 += preInput2[i];
        ...
        if(e.target.className == 'key--operator' && operatorCounter < 2){  //jika tombol operator diklik, isi variabel input1
            operator = e.target.dataset.action;
            operatorCounter ++; 


            if(clickCounter > 0){       //line ini mengecek apakah tombol sudah pernah diklik
                input1 = '';
                console.log('displayC: ',displayC);
                inputKey = displayC ;
                // operator = e.target.dataset.action;
            ...
            console.log('input1: ',input1);
            console.log('opCounter: ',operatorCounter);
    }
});
